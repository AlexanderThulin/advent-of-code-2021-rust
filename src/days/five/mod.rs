pub fn main() {
	...
	let res_one = map_one.iter().fold(0, |sum, x| sum + x.iter().filter(|p| **p > 1).collect::<Vec<_>>().len());
	...
	let res_two = map_two.iter().fold(0, |sum, x| sum + x.iter().filter(|p| **p > 1).collect::<Vec<_>>().len());
