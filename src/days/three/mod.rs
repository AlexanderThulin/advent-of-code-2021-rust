pub fn main() {
	let binary_map = include_str!("input.txt")
		...
		let gamma_string = sums
			.iter()
			.map(|sum| {if (*sum as usize) > binary_map.len() / 2 { '1' } else { '0' }} )
			.collect::<String>();
		...
		for i in 0..width {
			...
			let wanted = if (col_sum as f32) >= ((oxy_temp.len() as f32) / 2.0) { 1 } else { 0 };

			oxy_temp.retain(|row| row[i] == wanted);
			if oxy_temp.len() == 1 { break; }
		}

		for i in 0..width {
			let col_sum: u32 = co2_temp.iter().map(|arr| arr.iter().nth(i).unwrap()).sum();
			let wanted = if (col_sum as f32) >= ((co2_temp.len() as f32) / 2.0) { 0 } else { 1 };

			co2_temp.retain(|row| row[i] == wanted);
			if co2_temp.len() == 1 { break; }
		}

		let oxy = u32::from_str_radix(&oxy_temp[0].iter().map(|bit| bit.to_string()).collect::<String>(), 2).unwrap();
		let co2 = u32::from_str_radix(&co2_temp[0].iter().map(|bit| bit.to_string()).collect::<String>(), 2).unwrap();
